...
<p class="payment_module">
	<form action="{$paypal_usa_action|escape:'htmlall':'UTF-8'}" method="post">
		...
		{foreach from=$cart->getProducts() item=paypal_usa_product name="paypal_usa_products"}
			<input type="hidden" name="item_name_{$smarty.foreach.paypal_usa_products.index+1}" value="{$paypal_usa_product.name|escape:'htmlall':'UTF-8'}" />
			<input type="hidden" name="amount_{$smarty.foreach.paypal_usa_products.index+1}" value="{$paypal_usa_product.price_wt|floatval}" />
			<input type="hidden" name="quantity_{$smarty.foreach.paypal_usa_products.index+1}" value="{$paypal_usa_product.quantity|intval}" />
		{/foreach}
		{assign var="paypal_usa_total_shipping" value=$cart->getOrderTotal(true, Cart::ONLY_SHIPPING)}
		{if $paypal_usa_total_shipping}
			<input type="hidden" name="item_name_{$smarty.foreach.paypal_usa_products.index+2}" value="{l s='Shipping' mod='paypal_usa'}" />
			<input type="hidden" name="amount_{$smarty.foreach.paypal_usa_products.index+2}" value="{$paypal_usa_total_shipping|floatval}" />
			<input type="hidden" name="quantity_{$smarty.foreach.paypal_usa_products.index+2}" value="1">
		{/if}
